...
def clean_html_tags(text):
    """ฟังก์ชันเคลียร์แท็ก HTML ตกค้างพังๆ ป้องกันเลย์เอาต์หน้าเว็บฉีกขาด"""
    if not text or pd.isna(text): return ""
    cleaned = str(text)
    cleaned = re.sub(r'</?div[^>]*>', '', cleaned) # ลบ <div> หรือ </div>
    cleaned = re.sub(r'</?a[^>]*>', '', cleaned)   # ลบแท็กลิงก์ <a> ที่หลุดมา
    cleaned = re.sub(r'</?span[^>]*>', '', cleaned) # ลบแท็ก <span>
...
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            ...
            # คลีนข้อมูลตอนโหลดขึ้นมาแสดงผลเพื่อความปลอดภัย
            for r in records:
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_mv_milestone:
        ...
        # ปรับปรุงแถวแสดง Metric ให้อยู่คู่กันอย่างสวยงาม
...
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! ระบบล้างแท็ก HTML สไตล์เซฟตี้เปิดทำงานแล้วครับ")
        st.markdown("---")
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                if vid_submit and title.strip():
                    # สั่งล้างแท็ก HTML สกปรกออกจากหัวข้อและโน้ตก่อนบันทึกเสมอ
                    clean_title = clean_html_tags(title)
                    clean_note = clean_html_tags(note)
                    
                    item_data = {"title": clean_title, "date": str(date_val), "type": w_type, "link": link, "note": clean_note, "pinned": is_pinned}
                    if st.session_state.edit_index is not None:
