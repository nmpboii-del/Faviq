...
def fetch_youtube_meta(url):
    """ฟังก์ชันดึงทั้งชื่อคลิปและชื่อช่องอัตโนมัติผ่าน YouTube oEmbed API"""
    meta = {"title": "", "channel": ""}
...
def clean_html_tags(text):
    """ฟังก์ชันพิเศษล้างแท็กตกค้างพังๆ ในไฟล์ CSV เก่าออกให้หมด"""
    if not text or pd.isna(text): return ""
    cleaned = str(text)
    cleaned = cleaned.replace("</div>", "").replace("<div>", "")
    cleaned = re.sub(r'</?a[^>]*>', '', cleaned) # ลบแท็ก <a> หรือ </a> ที่เคยพังออกไป
...
def load_data():
    if os.path.exists(DATA_FILE):
        ...
        # คลีนข้อมูลเพื่อป้องกันตัวอักษรตกค้างจากประวัติเก่า
        records = df.to_dict('records')
        for r in records:
            r['title'] = clean_html_tags(r.get('title', ''))
            r['channel'] = clean_html_tags(r.get('channel', 'Official Channel'))
...
st.markdown(
    ...
    /* สไตล์การ์ดวิดีโอแบบคลีน ป้องกัน HTML แท็กซ้อนกันพัง */
    .yt-video-card {
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with tab_home:
        ...
        if pinned_vids:
            st.markdown('<div class="yt-shelf-title">📌 ผลงานแนะนำยอดนิยม / ห้ามพลาด</div>', unsafe_allow_html=True)
            ...
            for pv_idx, pv_item in enumerate(pinned_vids[:4]):
                with pv_cols[pv_idx % 4]:
                    ...
                    channel_name = pv_item.get('channel', 'Official Channel')
                    ...
                    st.markdown(f'''
                    <a class="yt-video-card" href="{click_url}" target="_blank">
                        <div class="yt-thumbnail-container">
                            <img class="yt-thumbnail-img" src="{thumb}">
                        </div>
                        <div class="yt-video-details">
                            <div class="yt-video-title">📌 {pv_item["title"]}</div>
                            <div class="yt-video-channel">{channel_name} • {pv_item["type"]}</div>
            ...
            for g_idx, g_item in enumerate(gifts_list[:4]):
                with g_home_cols[g_idx % 4]:
                    ...
                    <div class="gift-card">
                        ...
                        <div style="text-align:center; font-size:14px; font-weight:600; color:#f8fafc; margin-bottom:5px;">{pin_badge}{g_item["title"]}</div>
                ...
                if not df_shelf.empty:
                    ...
                    for v_idx, v_item in enumerate(df_shelf.to_dict('records')[:4]):
                        with v_cols[v_idx % 4]:
                            ...
                            channel_name = v_item.get('channel', 'Official Channel')
                            ...
                            <a class="yt-video-card" href="{click_url}" target="_blank">
                                ...
                                <div class="yt-video-details">
                                    ...
                                    <div class="yt-video-channel">{channel_name}</div>
    ...
    with tab_videos:
        ...
        for shelf in video_shelves:
            ...
            with col_sort:
                sort_order = st.selectbox("เรียงลำดับ:", ["ใหม่ล่าสุด", "เก่าที่สุด"], key=f"sort_{shelf['type'].replace(' ', '_')}")
    ...
    with tab_letters:
        ...
        with st.form(key="fan_message_form_v11", clear_on_submit=True):
...
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! โหมดคลีนข้อมูลตกค้างเปิดใช้งานแล้วครับ")
        st.markdown("---")
        ...
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ"):
            ...
            with col_g1:
                ...
                with st.form(key="add_gift_exp_v11", clear_on_submit=False):
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                if is_edit_mode:
                    ...
                    d_title, d_link, d_note, d_channel = "", "", "", "Official Channel"
                    d_pinned = False
                    d_date = datetime.date.today()
                    d_type_idx = 0
                    btn_txt = "🚀 อัปโหลดเข้าคลัง"
                
                # ตัวแปรสร้างเอกลักษณ์ป้องกัน Session State ล็อกเบิ้ลดึงข้อมูลครั้งเดียว
                state_form_suffix = f"edit_{st.session_state.edit_index}" if is_edit_mode else "add_new"
                link_key = f"yt_link_field_{state_form_suffix}"
                
                # รับค่าลิงก์แบบอิสระ
                link = st.text_input("ลิงก์คลิป (วางลิงก์ YouTube เพื่อดึงข้อมูลออโต้):", value=d_link, key=link_key)
                
                # ระบบตรวจจับหากมีการเปลี่ยนแปลงหรือกดวางลิงก์ใหม่ ให้ดึงใหม่ได้ตลอดเวลา
                last_check_key = f"prev_link_state_{state_form_suffix}"
                if link and (st.session_state.get(last_check_key, "") != link):
                    st.session_state[last_check_key] = link
                    with st.spinner("กำลังดึงข้อมูลอัปเดตจาก YouTube..."):
                        fetched_meta = fetch_youtube_meta(link)
                        if fetched_meta["title"]: d_title = fetched_meta["title"]
                        if fetched_meta["channel"]: d_channel = fetched_meta["channel"]
                
                with st.form(key=f'admin_vid_form_v11_{state_form_suffix}'):
                    title = st.text_input("ชื่อคลิป (พิมพ์แก้ไขเพิ่มได้):", value=d_title)
                    channel = st.text_input("ชื่อช่องต้นทาง (แก้ไขได้):", value=d_channel)
                ...
                if vid_submit and title.strip():
                    # ล้าง HTML บั๊กออกก่อนเซฟลง CSV ทุกครั้ง
                    clean_title = clean_html_tags(title)
                    clean_channel = clean_html_tags(channel)
                    clean_note = clean_html_tags(note)
                    
                    item_data = {"title": clean_title, "channel": clean_channel if clean_channel else "Official Channel", "date": str(date_val), "type": w_type, "link": link, "note": clean_note, "pinned": is_pinned}
                    ...
                    # เคลียร์ค่าตรวจจับเพื่อเปิดให้ดึงในครั้งถัดๆ ไปอย่างอิสระ
                    if last_check_key in st.session_state: del st.session_state[last_check_key]
                    
                    save_data(st.session_state.schedules)
                    st.success("บันทึกข้อมูลเรียบร้อยและล้างบั๊กเก่าแล้ว!")
                    st.rerun()
                    
                if is_edit_mode:
                    if st.button("❌ Cancel ยกเลิกการแก้ไข"): 
                        if last_check_key in st.session_state: del st.session_state[last_check_key]
                        st.session_state.edit_index = None
                        st.rerun()
            with col_v_manage:
                ...
                for idx, item in enumerate(st.session_state.schedules):
                    ...
                    v_c1.write(f"{idx+1}. {item['title']} ({item.get('channel', 'Official Channel')})")
        ...
        with st.expander("📬 5. เปิดกล่องอ่านจดหมายลับจากแฟนคลับ"):
