...
# --- ฟังก์ชันระบบดึงค่าและจัดการข้อมูล ---
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_shelves_area:
        # 🌟 ชั้นวางที่ 1: 🎨 ของแจกสำหรับแฟนๆ (แสดงรูปอัปเดตรหัส Base64 สำเร็จ)
    ...
    with col_letter_area:
        ...
        with st.form(key="fan_message_form_v7", clear_on_submit=True):
...
# ==========================================
# ⚙️ ระบบหลังบ้านจัดการข้อมูล (สำหรับแนท) - แบบ Full Edit Mode
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! โหมดแก้ไขข้อมูลทุกหมวดหมู่เปิดใช้งานแล้วครับแนท")
        st.markdown("---")
        
        # คอนโทรล Session State สำหรับดัชนีแก้ไข
        ...
        # 🎨 3. จัดการรูปภาพแจกฟรี (Full Edit Mode ✨)
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ (คลิกเพื่อแก้ไข/เพิ่ม/ลบ)"):
            gifts_data = load_gifts()
            col_g1, col_g2 = st.columns([1, 1.2])
            
            with col_g1:
                # ตรวจสอบสถานะว่ากำลังเพิ่มใหม่ หรือกำลังแก้ไขของแจกเดิม
                ...
                with st.form(key="add_gift_exp_v7", clear_on_submit=False):
                    g_title = st.text_input("ชื่อของแจก:", value=default_g_title)
                    uploaded_img_file = st.file_uploader("เลือกรูปภาพตัวอย่างใหม่ (ปล่อยว่างได้ถ้าไม่ต้องการเปลี่ยนรูปภาพ):", type=["png", "jpg", "jpeg"])
                    g_down_url = st.text_input("ลิงก์ดาวน์โหลดรูปเต็ม (Google Drive):", value=default_g_down)
                    gift_submit = st.form_submit_button(gift_gift_btn_txt if 'gift_gift_btn_txt' in locals() else gift_btn_txt)
                
                if gift_submit and g_title and g_down_url:
                    base64_img = None
                    if uploaded_img_file:
                        base64_img = base64.b64encode(uploaded_img_file.getvalue()).decode()
                    ...
                    if st.session_state.edit_gift_index is not None:
                        ...
                        if base64_img:
                            gifts_data[st.session_state.edit_gift_index]['img_url'] = base64_img
                        st.session_state.edit_gift_index = None
                        st.success("อัปเดตข้อมูลของแจกสำเร็จ!")
                    else:
                        # เพิ่มของแจกชิ้นใหม่
                        if uploaded_img_file:
                            gifts_data.append({"title": g_title, "img_url": base64_img, "download_url": g_down_url})
                            st.success("เพิ่มของแจกชิ้นใหม่เรียบร้อย!")
                        else:
                            st.warning("กรุณาเลือกไฟล์รูปภาพตัวอย่างสำหรับของแจกชิ้นใหม่ด้วยครับ")
                            
                    save_gifts(gifts_data)
                    st.rerun()
                    
                if st.session_state.edit_gift_index is not None:
                    if st.button("❌ ยกเลิกการแก้ไขของแจก"):
                        st.session_state.edit_gift_index = None
                        st.rerun()
            ...
            with col_g2:
                ...
                for g_i, g_item in enumerate(gifts_data):
                    g_c1, g_c2, g_c3 = st.columns([2.5, 0.8, 0.7])
                    g_c1.write(f"{g_i + 1}. {g_item['title']}")
                    if g_c2.button("📝 แก้ไข", key=f"edit_g_{g_i}"):
                        st.session_state.edit_gift_index = g_i
                        st.rerun()
                    if g_c3.button("ลบ", key=f"del_g_{g_i}"):
                        if st.session_state.edit_gift_index == g_i: st.session_state.edit_gift_index = None
                        gifts_data.pop(g_i)
                        save_gifts(gifts_data)
                        st.rerun()

        # 🎬 4. จัดการคลังวิดีโอทั่วไป (มีระบบแก้ไขเดิมอยู่แล้ว)
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                with st.form(key='admin_vid_exp_v7'):
                ...
                if st.session_state.edit_index is not None:
                    if st.button("❌ ยกเลิกแก้ไขวิดีโอ"): st.session_state.edit_index = None; st.rerun()
                        
            with col_v_manage:
                st.markdown("**📋 รายการวิดีโอปัจจุบัน**")
                for idx, item in enumerate(st.session_state.schedules):
                    v_c1, v_c2, v_c3 = st.columns([2.5, 0.8, 0.7])
                    v_c1.write(f"{idx+1}. {item['title']} ({item['type']})")
                    if v_c2.button("📝 แก้ไข", key=f"edit_v_{idx}"): st.session_state.edit_index = idx; st.rerun()
