...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_shelves_area:
        ...
        # --------------------------------------------------
        # 🚀 [โซนแนะนำพิเศษ] คัดเฉพาะรายการที่โดนปักหมุดมาโชว์ก่อนใคร
        # --------------------------------------------------
        gifts_list = load_gifts()
        all_vids = load_data()
        
        # กรองข้อมูลที่ถูกปักหมุด
            ...
            if pinned_gifts:
                ...
                for pg_idx, pg_item in enumerate(pinned_gifts):
                    with pg_cols[pg_idx % 4]:
                        ...
                        st.markdown(f'<div class="gift-card" style="border: 2px solid #f59e0b;"><img class="gift-img" src="{img_src}"><div class="video-title" style="text-align:center; color:#f59e0b;">📌 {pg_item["title"]}</div><a class="download-btn" style="background-color:#f59e0b; color:#0f172a !important;" href="{pg_item["download_url"]}" target="_blank">📥 โหลดรูปเต็ม</a></div>', unsafe_allow_html=True)
        ...
        # --------------------------------------------------
        # 🌟 ชั้นวางปกติที่ 1: 🎨 ของแจกสำหรับแฟนๆ ทั้งหมด
        # --------------------------------------------------
        ...
        if not gifts_list: st.caption("ขณะนี้ยังไม่มีรูปภาพของแจกเปิดให้ดาวน์โหลด")
            ...
            for g_idx, g_item in enumerate(display_gifts):
                with g_cols[g_idx % 4]:
                    ...
                    if img_src and not str(img_src).startswith("http"): img_src = f"data:image/png;base64,{img_src}"
                    st.markdown(f'<div class="gift-card"><img class="gift-img" src="{img_src}"><div class="video-title" style="text-align:center;">{g_item["title"]}</div><a class="download-btn" href="{g_item["download_url"]}" target="_blank">📥 โหลดรูปเต็ม</a></div>', unsafe_allow_html=True)
            if len(gifts_list) > 4:
...
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! ระบบปักหมุดด่วนและปุ่มแก้ไขพร้อมใช้งานครบทุกฟังก์ชันครับแนท")
        st.markdown("---")
        ...
        # 🎨 3. จัดการรูปภาพแจกฟรี (เพิ่มระบบปักหมุดด่วน ✨)
        ...
        # 🎬 4. จัดการคลังวิดีโอทั่วไป (มีปักหมุดด่วนและฟอร์มแก้ไขครบถ้วน ✨)
