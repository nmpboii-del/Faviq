...
# --- ตกแต่ง CSS หน้าเว็บปรับปรุงเรื่องรูปกึ่งกลางอย่างสมบูรณ์ ---
st.markdown(
    """
    <style>
    ...
    /* สไตล์สำหรับการ์ดแจกรูปภาพแก้บั๊กรูปแปลกให้อยู่กึ่งกลางเป๊ะ */
    .gift-card { background-color: #0f172a; border: 1px solid #334155; border-radius: 14px; padding: 15px; text-align: center; margin-bottom: 15px; display: block;}
    .gift-img-container { width: 100%; height: 180px; display: table; margin-bottom: 8px; background-color: #020617; border-radius: 10px; overflow: hidden; }
    .gift-img-cell { display: table-cell; vertical-align: middle; text-align: center; }
    .gift-img { max-width: 100%; max-height: 180px; object-fit: contain; display: inline-block; border-radius: 6px; }
    
    .watch-btn { display: inline-block; background-color: #ef4444; color: white !important; padding: 4px 12px; font-size: 12px; font-weight: bold; border-radius: 20px; text-decoration: none !important; margin-top: 6px; text-align: center; }
    ...
    .shelf-title { font-size: 18px; font-weight: bold; color: #38bdf8; margin: 25px 0 10px 0; display: flex; align-items: center; gap: 8px; }
    .pinned-shelf-title { font-size: 18px; font-weight: bold; color: #f59e0b; margin: 25px 0 10px 0; display: flex; align-items: center; gap: 8px; }
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_shelves_area:
        ...
        # --------------------------------------------------
        # 🚀 [โซนแนะนำพิเศษ] คัดเฉพาะรายการที่โดนปักหมุด
        # --------------------------------------------------
        ...
        if pinned_gifts or pinned_vids:
            st.markdown("<h3 style='color: #f59e0b; margin-top:10px;'>📌 ข้อมูลแนะนำพิเศษ (Pinned Highlights)</h3>", unsafe_allow_html=True)
            
            # 1. แถวของแจกปักหมุด (แก้ไขจัดตำแหน่งรูปให้กลางเป๊ะ)
            if pinned_gifts:
                st.markdown('<div class="pinned-shelf-title">⭐ ของแจกแนะนำพิเศษ</div>', unsafe_allow_html=True)
                pg_cols = st.columns(4)
                for pg_idx, pg_item in enumerate(pinned_gifts):
                    with pg_cols[pg_idx % 4]:
                        img_src = pg_item['img_url']
                        if img_src and not str(img_src).startswith("http"): img_src = f"data:image/png;base64,{img_src}"
                        st.markdown(f"""
                        <div class="gift-card" style="border: 2px solid #f59e0b;">
                            <div class="gift-img-container">
                                <div class="gift-img-cell">
                                    <img class="gift-img" src="{img_src}">
                                </div>
                            </div>
                            <div class="video-title" style="text-align:center; color:#f59e0b;">📌 {pg_item["title"]}</div>
                            <a class="download-btn" style="background-color:#f59e0b; color:#0f172a !important;" href="{pg_item["download_url"]}" target="_blank">📥 โหลดรูปเต็ม</a>
                        </div>
                        """, unsafe_allow_html=True)
            
            # 2. แถววิดีโอปักหมุด
            if pinned_vids:
                st.markdown('<div class="pinned-shelf-title">⭐ ผลงานยอดนิยม / ห้ามพลาด</div>', unsafe_allow_html=True)
                ...
                for pv_idx, pv_item in enumerate(pinned_vids):
                    with pv_cols[pv_idx % 4]:
                        ...
                        st.markdown(f'<div class="video-card" style="border: 2px solid #f59e0b;"><div class="thumbnail-container"><img class="thumbnail-img" src="{thumb}"></div><div class="video-title" style="color:#f59e0b;">📌 {pv_item["title"]}</div><div class="video-meta">📅 {pv_item["date"]} • {pv_item["type"]}</div>', unsafe_allow_html=True)
                        if pv_item['note'] and not pd.isna(pv_item['note']): st.markdown(f'<div style="font-size:12px; color:#cbd5e1; font-style:italic; margin-top:4px;">💬 {pv_item["note"]}</div>', unsafe_allow_html=True)
                        if pv_item['link'] and not pd.isna(pv_item['link']): st.markdown(f'<a class="watch-btn" style="background-color:#f59e0b; color:#0f172a !important;" href="{pv_item["link"]}" target="_blank">▶ ชมคลิป</a>', unsafe_allow_html=True)
                        st.markdown("</div>", unsafe_allow_html=True)
            st.markdown("<hr style='border-color: #f59e0b; opacity:0.3;'>", unsafe_allow_html=True)

        # --------------------------------------------------
        # 🌟 ชั้นวางปกติที่ 1: 🎨 ของแจกสำหรับแฟนๆ ทั้งหมด (จัดตำแหน่งรูปกึ่งกลางเป๊ะ)
        # --------------------------------------------------
        st.markdown('<div class="shelf-title">🎨 แจกรูปภาพและของสมนาคุณสำหรับแฟนๆ ทั้งหมด</div>', unsafe_allow_html=True)
        ...
        # ชั้นวางวิดีโอปกติแยกตามหมวดหมู่
        ...
        for shelf in video_shelves:
            ...
            if df_vids.empty: st.caption("ยังไม่มีข้อมูลวิดีโอในคลัง")
            else:
                df_shelf = df_vids[df_vids['type'] == shelf['type']]
                if df_shelf.empty: st.caption("ยังไม่มีวิดีโอในหมวดหมู่นี้")
                else:
                    ...
                    for v_idx, v_item in enumerate(display_vids):
                        with v_cols[v_idx % 4]:
                            ...
                            st.markdown(f'<div class="video-card"><div class="thumbnail-container"><img class="thumbnail-img" src="{thumb}"></div><div class="video-title">{v_item["title"]}</div><div class="video-meta">📅 {v_item["date"]}</div>', unsafe_allow_html=True)
    ...
    with col_letter_area:
        ...
        if submit_letter and fan_msg.strip():
            ...
            st.success("💖 ส่งจดหมายสำเร็จ!")

# ==========================================
# ⚙️ ระบบหลังบ้านจัดการข้อมูล (สำหรับแนท) - Full Edit Mode + Pinned Everywhere
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! โหมดแก้ไขรูปภาพและการจัดวางกึ่งกลางพร้อมทำงานแล้วครับแนท")
        st.markdown("---")
        ...
        # 🎨 3. จัดการรูปภาพแจกฟรี (Full Edit Mode + Fix บั๊กอัปโหลด)
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ (แก้ไข/ปักหมุด/ลบ)"):
            ...
            with col_g2:
                ...
                for g_i, g_item in enumerate(gifts_data):
                    ...
                    if g_c2.button(g_pin_lbl, key=f"quick_pin_g_{g_i}"):
                        g_item['pinned'] = not g_pin_val
                        save_gifts(gifts_data); st.rerun()
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_manage:
                ...
                for idx, item in enumerate(st.session_state.schedules):
                    ...
                    if v_c2.button(v_pin_lbl, key=f"quick_pin_v_{idx}"):
                        item['pinned'] = not v_pin_val
                        save_data(st.session_state.schedules); st.rerun()
