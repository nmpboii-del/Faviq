...
def fetch_youtube_details(url):
    """ฟังก์ชันอัปเกรดใหม่: ดึงทั้งชื่อคลิปและชื่อช่องจาก YouTube แบบทนทานพิเศษ"""
    video_id = extract_youtube_id(url)
    if not video_id:
        return "วิดีโอไม่ระบุชื่อ", "Unknown Channel"
    
    # วิธีที่ 1: ดึงผ่าน oEmbed API (แม่นยำและปลอดภัยจากการโดนบล็อกที่สุดสำหรับชื่อคลิปและช่อง)
    try:
    ...
    # วิธีที่ 2: ดึงจากหน้าเว็บตรงๆ (Fallback หากวิธีแรกพัง)
    try:
        watch_url = f"https://www.youtube.com/watch?v={video_id}"
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "Accept-Language": "th-TH,th;q=0.9"}
        res = requests.get(watch_url, headers=headers, timeout=5)
        if res.status_code == 200:
            title_match = re.search(r'<title>(.*?)</title>', res.text)
            title = title_match.group(1).replace(" - YouTube", "").strip() if title_match else "วิดีโอ YouTube"
            
            # ค้นหาชื่อช่องจากสคริปต์หลังบ้าน YouTube
            channel_match = re.search(r'"ownerChannelName":"([^"]+)"', res.text)
            if not channel_match:
                channel_match = re.search(r'"author":"([^"]+)"', res.text)
            channel = channel_match.group(1).strip() if channel_match else "Official Channel"
            
            return title, channel
    except:
        pass
        
    return "วิดีโอ YouTube", "Official Channel"
...
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            ...
            # ถ้าไม่มีคอลัมน์ channel ให้สร้างขึ้นมากันพัง
...
st.markdown(
    ...
    .yt-video-channel {
        font-size: 12px;
        color: #ef4444; /* ไฮไลท์สีแดงแชนเนลแบบเท่ๆ */
        font-weight: 500;
        margin-bottom: 2px;
    }
    .yt-video-meta {
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_mv_milestone:
        ...
        col_metric1.metric(label="📈 ยอดวิวบน YouTube", value=f"{c_views:,} วิว")
        col_metric2.metric(label="🎯 เป้าหมาย", value=f"{t_views:,} วิว")
...
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        ...
        # 🎬 4. จัดการคลังวิดีโอทั่วไป (จุดแก้ไขหลัก)
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            
            # ปุ่มดึงข้อมูลอัตโนมัติจากลิงก์ YouTube เพื่อช่วยเติมฟอร์มให้แนท
            ...
            if st.button("🔍 ดึงชื่อคลิปและชื่อช่องอัตโนมัติ"):
                if yt_fetch_link.strip():
                    with st.spinner("กำลังคุยกับ YouTube หลังบ้าน..."):
                        fetched_title, fetched_channel = fetch_youtube_details(yt_fetch_link)
                        ...
                        st.success("ดึงข้อมูลสำเร็จ! ข้อมูลถูกนำไปเตรียมไว้ในฟอร์มด้านล่างแล้ว")
                else:
            ...
            with col_v_form:
                if st.session_state.edit_index is not None:
                    ...
                    # เรียกใช้ค่าที่ดึงมาจาก YouTube อัตโนมัติ (ถ้ามี)
                ...
                if vid_submit and title.strip():
                    ...
                    # ล้างค่ากล่องสเตทชั่วคราวออกไป
                    for k in ["temp_fetched_title", "temp_fetched_channel", "temp_fetched_link"]:
                        if k in st.session_state: del st.session_state[k]
                        
                    save_data(st.session_state.schedules)
                    st.success("บันทึกเข้าคลังและอัปเดตชื่อช่องเรียบร้อย!")
                    st.rerun()
            ...
            with col_v_manage:
                ...
                for idx, item in enumerate(st.session_state.schedules):
                    ...
                    v_c1.write(f"{idx+1}. {item['title']} \n<span style='color:#ef4444; font-size:12px;'>ช่อง: {item.get('channel', 'Official Channel')}</span>", unsafe_allow_html=True)
