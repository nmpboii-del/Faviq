...
def fetch_youtube_meta(url):
    ...
    try:
        # ใช้ oEmbed API ของ YouTube เพื่อเลี่ยงการโดนบล็อกบอท
...
st.markdown(
    ...
    /* สไตล์การ์ดวิดีโอใหม่ป้องกัน String บั๊ก */
    .yt-video-card {
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with tab_letters:
        ...
        with st.form(key="fan_message_form_v10", clear_on_submit=True):
...
# ==========================================
    ...
    if password_input == ADMIN_PASSWORD:
        st.success("🔓 ยืนยันตัวตนสำเร็จ! โหมดแกลเลอรีแก้ไขบั๊ก HTML เรียบร้อยแล้วครับแนท")
        st.markdown("---")
        ...
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ"):
            ...
            with col_g1:
                ...
                with st.form(key="add_gift_exp_v10", clear_on_submit=False):
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                # ช่องกรอกลิงก์
                link_input_key = f"link_input_{st.session_state.edit_index}"
                link = st.text_input("ลิงก์คลิป (วางลิงก์ YouTube ระบบจะดึงชื่อคลิปและชื่อช่องอัตโนมัติ):", value=d_link, key=link_input_key)
                
                # ระบบดึงข้อมูล Reactive อัปเดตชื่อคลิปและชื่อช่องพร้อมกัน
                if link and (link != st.session_state.get("last_fetched_link", "")):
                    st.session_state.last_fetched_link = link
                    with st.spinner("กำลังดึงชื่อคลิปและชื่อช่องจาก YouTube..."):
                ...
                with st.form(key='admin_vid_form_v10'):
                    title = st.text_input("ชื่อคลิป (แก้ไขปรับแต่งเพิ่มได้):", value=d_title)
                ...
                if vid_submit and title.strip():
                    item_data = {"title": title, "channel": channel if channel.strip() else "Official Channel", "date": str(date_val), "type": w_type, "link": link, "note": note, "pinned": is_pinned}
                    if st.session_state.edit_index is not None:
