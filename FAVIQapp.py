    ...
    if os.path.exists(MV_FILE):
        try:
            ...
            if (current_time - float(data.get('last_updated', 0.0))) >= 3600:
                ...
                # จำลองการดึงวิวง่ายๆ หรือสามารถผูกสูตรดึงยอดวิวสดได้ที่นี่
                fresh = None 
                if fresh:
...
def load_data():
    """โหลดข้อมูลและทำความสะอาด HTML ค้างคา (แก้ไขจุด Return เรียบร้อย)"""
    if os.path.exists(DATA_FILE):
        try:
            ...
            return records  # แก้ไขเพิ่มคำสั่งส่งค่าคืนตรงนี้ ข้อมูลจะกลับมาปกติครับ!
        except:
            return []
    return []
...
# ดึงข้อมูลเข้า Session State ทุกครั้งที่รัน
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with tab_letters:
        ...
        with st.form(key="fan_message_form_v12", clear_on_submit=True):
    ...
    if password_input == ADMIN_PASSWORD:
        ...
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ"):
            ...
            with col_g1:
                ...
                with st.form(key="add_gift_exp_v12", clear_on_submit=False):
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                with st.form(key=f'admin_vid_form_v12_{state_form_suffix}'):
            ...
            with col_v_manage:
                ...
                if not st.session_state.schedules:
                    st.caption("ไม่มีข้อมูลวิดีโอในระบบ")
                else:
