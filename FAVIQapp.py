...
st.markdown(
    ...
    /* สไตล์การ์ดแถวแนวตั้งแบบดั้งเดิม (List Row) */
    .vertical-list-row {
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
    ...
    with col_shelves_area:
        ...
        # 🚀 โซนแนะนำพิเศษ (Pinned Highlights)
            ...
            if pinned_gifts:
                ...
                for pg_idx, pg_item in enumerate(pinned_gifts):
                    with pg_cols[pg_idx % 4]:
                        ...
                        st.markdown(f'<div class="gift-card" style="border: 2px solid #f59e0b;"><div class="gift-img-container"><img class="gift-img" src="{img_src}"></div><div class="video-title" style="text-align:center; color:#f59e0b;">📌 {pg_item["title"]}</div><a class="download-btn" style="background-color:#f59e0b; color:#0f172a !important;" href="{pg_item["download_url"]}" target="_blank">📥 โหลดรูปเต็ม</a></div>', unsafe_allow_html=True)
            if pinned_vids:
        ...
        # ชั้นวางวิดีโอปกติแยกตามหมวดหมู่ (แนวนอนสไตล์ YouTube)
        video_shelves = [
