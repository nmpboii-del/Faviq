...
def fetch_youtube_title(url):
    """ฟังก์ชันดึงชื่อคลิปอัตโนมัติจากแท็กเปิดของ YouTube"""
    if not url: return ""
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Accept-Language": "th-TH,th;q=0.9,en;q=0.8"}
...
st.markdown(
    ...
    /* สไตล์การ์ดวิดีโอแบบป้องกันแท็กค้างหลุด */
    .yt-video-card {
    ...
    /* ทำตัวคลิกครอบใสป้องซ้อนกันเออร์เรอร์ */
    .yt-click-overlay {
        position: absolute;
...
if view_mode == "🏠 หน้าแรกแกลเลอรี":
            ...
            for pv_idx, pv_item in enumerate(pinned_vids[:4]):
                with pv_cols[pv_idx % 4]:
                    ...
                    <div class="yt-video-card">
                        <a href="{click_url}" target="_blank" class="yt-click-overlay"></a>
    ...
    with tab_letters:
        ...
        with st.form(key="fan_message_form_v9", clear_on_submit=True):
    ...
    if password_input == ADMIN_PASSWORD:
        ...
        with st.expander("🎨 3. จัดการรูปภาพแจกฟรีให้แฟนคลับ (แก้ไข/ปักหมุด/ลบ)"):
            ...
            with col_g1:
                ...
                with st.form(key="add_gift_exp_v9", clear_on_submit=False):
        ...
        with st.expander("🎬 4. จัดการคลังผลงานวิดีโอและคิวงานทั่วไป"):
            ...
            with col_v_form:
                ...
                # ระบบตรวจจับลิงก์แบบ Reactive ดึงชื่ออัตโนมัติ
                link_input_key = f"link_input_{st.session_state.edit_index}"
                link = st.text_input("ลิงก์คลิป (วางลิงก์ YouTube เพื่อดึงชื่ออัตโนมัติ):", value=d_link, key=link_input_key)
                
                # ถ้าเป็นโหมดเพิ่มใหม่ และแนทเพิ่งวางลิงก์ แต่ช่องชื่อยังว่างอยู่ ให้ดึงชื่อให้อัตโนมัติ
                if link and not d_title and "last_fetched_link" not in st.session_state:
                    st.session_state.last_fetched_link = link
                    with st.spinner("กำลังดึงชื่อคลิปจาก YouTube..."):
                        d_title = fetch_youtube_title(link)
                elif link and st.session_state.get("last_fetched_link") != link:
                    st.session_state.last_fetched_link = link
                    with st.spinner("กำลังอัปเดตดึงชื่อคลิปใหม่..."):
                        d_title = fetch_youtube_title(link)
                
                with st.form(key='admin_vid_form_v9'):
                    title = st.text_input("ชื่อคลิป (สามารถกดลบพิมพ์แก้ไขได้):", value=d_title)
